...
class graph {
  ...
  public:
    ...
    bool dijkstraHEAP(int src, std::vector<vertex_label> &report) {
      int u, v;
      double min_d;
      int next_vertex;
      pqueue q(num_nodes());
      ...
      while(q.size() > 0) {
          ...
          for(edge &e : vertices[u].outgoing) {
              ...
              else if(dist < report[v].dist) {
                  if(!q.change_priority(v, dist) )
                      printf("BUGGGG 2\n");  // if shorter path found to v, then v had
                  // better be in the priority queue
                  report[v].dist = dist;
                  report[v].pred = u;
              }
